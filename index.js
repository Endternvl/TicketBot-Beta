...
client.on("ready", () => {
    console.log(`Bot has started, with ${client.users.size} users, in ${client.channels.size} channels of ${client.guilds.size} guilds. Credits to Skaryey (aka. Endternvl)`);
    client.user.setActivity(`TICKET BOT BETA V1 BY SKARYEY`);
  });
  ...
  client.on("message", async message => {
    ...
    if (message.channel.type !== "dm") {
        if (message.channel.name.indexOf('ticket') > -1 && command != "close") {
            ...
            if (command == "close" && message.channel.name.indexOf('ticket') > -1) {
                ...
                const embed = new RichEmbed()
                    .setColor(0xCF40FA)
                    .addField(`New message`, "Your request has been closed, no need to reply to this message")
                    .setTimestamp();
                pq.send({
                    embed: embed
                });
    ...
    if (message.channel.type === "dm") {
        ...
        server.createChannel(`ticket-${message.author.id}`, "text").then(c => {
            ...
            message.channel.send(`:white_check_mark: Please wait staff will comes shortly.`);
            const embed = new RichEmbed()
                .setColor(0xCF40FA)
                .addField(`Ticket of ${message.author.username}#${message.author.discriminator}`, message.content)
                .setTimestamp();
            c.send({
                embed: embed
            });
